...
import { AgentTreeBridge } from './ws-bridge';
import { setupCaptchaDetection } from './captcha-detector';
...
// --- AgentTree WebSocket Bridge ---
const agentTreeBridge = new AgentTreeBridge(browserContext, 9223);
agentTreeBridge.start();

// Setup CAPTCHA detection on all navigations
setupCaptchaDetection((tabId, patterns) => {
  logger.info('CAPTCHA detected in tab', tabId, patterns);
  agentTreeBridge.reportCaptcha(tabId, patterns);
});
